...
from googleapiclient.errors import HttpError as GcalHttpError


# ==============================
# ✅ Transient-error retry helpers
# ==============================
def _request_with_retry(method, url, attempts=4, backoff=5, **kwargs):
    """
    requests 호출 공통 래퍼: 429/5xx/연결 오류는 지수 백오프로 재시도.
    4xx(429 제외)는 즉시 raise (재시도 의미 없음).
    """
    last = None
...
def _gcal_execute_with_retry(req, attempts=4, backoff=5):
    """
    googleapiclient .execute() 래퍼: 429/5xx는 백오프 재시도, 그 외 HttpError는 즉시 raise.
    """
    last = None
    for i in range(attempts):
        try:
            return req.execute()
        except GcalHttpError as e:
...
def update_notion_page(page_id: str, props: dict):
    ...
    resp = _request_with_retry("patch", url, headers=headers, json=payload)
    resp.raise_for_status()
    return resp.json()
...
def fetch_gcal_events_for_date(service, calendar_id: str, date_obj):
    ...
    while True:
        ...
        res = _gcal_execute_with_retry(req)
...
def send_new_message(webhook_url, payload):
    ...
    r = _request_with_retry("post", base, params={"wait": "true"}, json=payload)
    r.raise_for_status()
    return r.json()["id"]
...
def edit_message(webhook_url, message_id, payload):
    ...
    r = _request_with_retry("patch", url, json=payload)
    r.raise_for_status()
    return True
